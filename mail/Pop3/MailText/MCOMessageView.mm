    ...
    if (array.count > 0) {
        ...
        for(int i = 0;i < array.count; i++){
            MCOAbstractPart *attach = [array objectAtIndex:i];
             ...
             BOOL eixtFlag = [[NSFileManager defaultManager] fileExistsAtPath:[Util getFileAtDocumentPath:attach.filename]];
            
            if (i == 0) {
                if (eixtFlag) {
                    fujianStr = [NSString stringWithFormat:@"<div><b>附件:<b/><b>%@</b>&nbsp<a id = \"FJID%i\" href=\"http://OpenFile/%d\">打开</a></div>",attach.filename,i,i];
                }else
                    fujianStr = [NSString stringWithFormat:@"<div><b>附件:<b/><b>%@</b>&nbsp<a id = \"FJID%i\" href=\"http://DownLoadFJ/%d\">下载</a></div>",attach.filename,i,i];
            }else{
                
                NSString *sec = nil;
                if (eixtFlag) {
                    sec = [NSString stringWithFormat:@"<div>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<b>%@</b>&nbsp<a id = \"FJID%i\" href=\"http://OpenFile/%d\">打开</a></div>",attach.filename,i,i];
                }else
                    sec = [NSString stringWithFormat:@"<div>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<b>%@</b>&nbsp<a id = \"FJID%i\" href=\"http://DownLoadFJ/%d\">下载</a></div>",attach.filename,i,i];
                fujianStr = [NSString stringWithFormat:@"%@%@",fujianStr,sec];
            }
        }
        [muContent replaceOccurrencesOfString:needRplace withString:fujianStr options:NULL range:NSMakeRange(0, muContent.length)];
        NSLog(@"------ gwj  = %@",muContent);
    }else{
        [muContent replaceOccurrencesOfString:needRplace withString:@"" options:NULL range:NSMakeRange(0, muContent.length)];
    }
    
    [muContent replaceOccurrencesOfString:@"<hr/>" withString:@"" options:nil range:NSMakeRange(0, muContent.length)];
    //end
    self.htmlString = muContent;
...
//        @"</iframe></html>", mainJavascript, mainStyle,self.htmlString];
...
{
    if ([self.delegate respondsToSelector:@selector(MessageViewStopLoading)]) {
        [self.delegate MessageViewStopLoading];
    }
}
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    NSLog(@"arrya = %@",array);
    if (array.count == 4) {
        if ([[[array objectAtIndex:2] lowercaseString] isEqualToString:@"downloadfj"]) {
            //[self changeTitleById:[array objectAtIndex:3]];
            if ([[self delegate] respondsToSelector:@selector(MessageView:downLoadFj:)]) {
                [[self delegate] MessageView:self downLoadFj:[array objectAtIndex:3]];
            }
            return NO;
        }
        else if ([[[array objectAtIndex:2] lowercaseString] isEqualToString:@"openfile"])//打开预览
        {
            if ([[self delegate] respondsToSelector:@selector(MessageView:preViewFj:)]) {
                [[self delegate] MessageView:self preViewFj:[array objectAtIndex:3]];
            }
            return NO;
        }
    }
...
- (NSString *) MCOAbstractMessage:(MCOAbstractMessage *)msg templateForAttachment:(MCOAbstractPart *)part
{
    ...
    NSLog(@"temlpate string = %@",templateString);
    templateString = [NSString stringWithFormat:@"<div id=\"{{CONTENTID}}\">%@</div>", templateString];
    return templateString;
}
...
#pragma mark add by gwj below content
- (void)changeTitleById:(NSString *)fjId
{
    NSString *target = [NSString stringWithFormat:@"<a id = \"FJID%@\" href=\"http://DownLoadFJ/%@\">下载</a>",fjId,fjId];
    NSString *result = [NSString stringWithFormat:@"<a id = \"FJID%@\" href=\"http://OpenFile/%@\">打开</a>",fjId,fjId];
    NSMutableString *t = [NSMutableString stringWithString:self.htmlString];
    [t replaceOccurrencesOfString:target withString:result options:NULL range:NSMakeRange(0, self.htmlString.length)];
